...
static void power_suspend(struct work_struct *work)
{
	...
	printk(KERN_DEBUG "[POWERSUSPEND] entering suspend...\n");
	#endif
	mutex_lock(&power_suspend_lock);
	...
	#ifdef POWER_SUSPEND_DEBUG
	printk(KERN_DEBUG "[POWERSUSPEND] suspending...\n");
	#endif
	...
	#ifdef POWER_SUSPEND_DEBUG
	printk(KERN_DEBUG "[POWERSUSPEND] suspend completed.\n");
	#endif
abort_suspend:
	mutex_unlock(&power_suspend_lock);
}
...
static void power_resume(struct work_struct *work)
{
	...
	#ifdef POWER_SUSPEND_DEBUG
	printk(KERN_DEBUG "[POWERSUSPEND] entering resume...\n");
	#endif
	mutex_lock(&power_suspend_lock);
	spin_lock_irqsave(&state_lock, irqflags);
	...
	#ifdef POWER_SUSPEND_DEBUG
	printk(KERN_DEBUG "[POWERSUSPEND] resuming...\n");
	#endif
	...
	#ifdef POWER_SUSPEND_DEBUG
	printk(KERN_DEBUG "[POWERSUSPEND] resume completed.\n");
	#endif
abort_resume:
	mutex_unlock(&power_suspend_lock);
}
...
void set_power_suspend_state(int new_state)
{
	...
	if (state != new_state) {
		...
		if (state == POWER_SUSPEND_INACTIVE && new_state == POWER_SUSPEND_ACTIVE) {
			#ifdef POWER_SUSPEND_DEBUG
			printk(KERN_DEBUG "[POWERSUSPEND] state activated.\n");
			#endif
			state = new_state;
			...
			#ifdef POWER_SUSPEND_DEBUG
			printk(KERN_DEBUG "[POWERSUSPEND] state deactivated.\n");
			#endif
			state = new_state;
		...
		printk(KERN_DEBUG "[POWERSUSPEND] state change requested, but unchanged ?! Ignored !\n");
	#endif
	}
}
...
{
	#ifdef POWER_SUSPEND_DEBUG
	printk(KERN_DEBUG "[POWERSUSPEND] autosleep resquests %s.\n", new_state == POWER_SUSPEND_ACTIVE ? "sleep" : "wakeup");
	#endif
...
{
	#ifdef POWER_SUSPEND_DEBUG
	printk(KERN_DEBUG "[POWERSUSPEND] panel resquests %s.\n", new_state == POWER_SUSPEND_ACTIVE ? "sleep" : "wakeup");
	#endif
	/* Yank555.lu : Only allow autosleep hook changes in autosleep & hybrid mode */
		...
		struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	#ifdef POWER_SUSPEND_DEBUG
	printk(KERN_DEBUG "[POWERSUSPEND] userspace resquests %s.\n", new_state == POWER_SUSPEND_ACTIVE ? "sleep" : "wakeup");
	#endif
...
static int __init power_suspend_init(void)
{
	...
	int sysfs_result;
        ...
        if (!power_suspend_kobj) {
                printk(KERN_ERR "%s kobject create failed!\n", __FUNCTION__);
                return -ENOMEM;
        }
        ...
        if (sysfs_result) {
                printk(KERN_DEBUG "%s group create failed!\n", __FUNCTION__);
