...
/*  vol,clk,min,max,down stay, pm_qos mem, pm_qos int, pm_qos cpu_kfc_min, pm_qos cpu_egl_max */
	...
	{1000000, 480, 80,  90, 1, 0, 825000, 400000, 1000000, 1600000},
#ifdef CONFIG_SOC_EXYNOS5422_REV_0
	{1037500, 543, 85,  92, 1, 0, 825000, 400000, 1000000, 1600000},
	{1050000, 600, 93,  95, 1, 0, 825000, 400000, 1000000, 1600000},
	{1075000, 677, 96, 100, 1, 0, 825000, 400000, 1000000, 1600000},
#else
...
int gpu_dvfs_governor_init(struct kbase_device *kbdev, int governor_type)
{
	...
	case G3D_DVFS_GOVERNOR_BOOSTER:
		...
		break;
	case G3D_DVFS_GOVERNOR_INTERACTIVE:
		gpu_dvfs_get_next_freq = (GET_NEXT_FREQ)&gpu_dvfs_governor_interactive;
		platform->table = gpu_dvfs_infotbl_default;
		platform->table_size = GPU_DVFS_TABLE_SIZE(gpu_dvfs_infotbl_default);
